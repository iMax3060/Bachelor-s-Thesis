...
bool bf_hashbucket<T>::find(PageID key, T& value) {
    ...
    for (uint32_t i = 0; i < _used_count 
                      && i < HASHBUCKET_INITIAL_CHUNK_SIZE; ++i) {
            ...
            cur_chunk = cur_chunk->next_chunk)
    {
        ...
        for (uint32_t i = 0; i < cur_chunk->size
                    && cur_count < _used_count; ++i, ++cur_count) {
...
bool bf_hashbucket<T>::update (PageID key, T value) {
    ...
    for (bf_hashbucket_chunk_linked<T>* cur_chunk
             = _chunk.next_chunk; 
         cur_count < _used_count;
         cur_chunk = cur_chunk->next_chunk) {
        w_assert1(cur_chunk != NULL);
        for (uint32_t i = 0; 
             i < cur_chunk->size && cur_count < _used_count;
             ++i, ++cur_count) {
            if (cur_chunk->keys[i] == key) {
...
bool bf_hashbucket<T>::append_if_not_exists (PageID key, T value) {
    ...
    for (bf_hashbucket_chunk_linked<T>* cur_chunk
             = _chunk.next_chunk;;
         cur_chunk = cur_chunk->next_chunk) {
        for (uint32_t i = 0; i < _used_count - cur_count
                   && i < cur_chunk->size; ++i) {
            if (cur_chunk->keys[i] == key) {
                return false;
            }
        ...
        if (cur_chunk->next_chunk == NULL) {
            cur_chunk->next_chunk
              = new bf_hashbucket_chunk_linked<T>
...
bool bf_hashbucket<T>::remove (PageID key) {
    ...
    if (found && _used_count > HASHBUCKET_INITIAL_CHUNK_SIZE) {
        _chunk.values[HASHBUCKET_INITIAL_CHUNK_SIZE - 1] 
            = _chunk.next_chunk->values[0];
        _chunk.keys[HASHBUCKET_INITIAL_CHUNK_SIZE - 1] 
            = _chunk.next_chunk->keys[0];
    }
    ...
    for (bf_hashbucket_chunk_linked<T>* cur_chunk 
             = _chunk.next_chunk;
         cur_count < _used_count; cur_chunk 
             = cur_chunk->next_chunk) {
        w_assert1(cur_chunk != NULL);
        for (uint32_t i = 0;
             i < cur_chunk->size && cur_count < _used_count;
        ...
        if (found && _used_count > cur_count) {
            cur_chunk->values[cur_chunk->size - 1] 
                = cur_chunk->next_chunk->values[0];
            cur_chunk->keys[cur_chunk->size - 1] 
                = cur_chunk->next_chunk->keys[0];
        }
    }
