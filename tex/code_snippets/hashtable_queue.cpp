...
bool hashtable_queue<key>::insert_back(key k) {
    if (!_direct_access_queue->empty()) {
        ...
        (*_direct_access_queue)[k]
                   = key_pair(old_back, _invalid_key);
        ...
        (*_direct_access_queue)[k]
                   = key_pair(_invalid_key, _invalid_key);
        _back = k;
        _front = k;
...
bool hashtable_queue<key>::remove_front() {
    if (_direct_access_queue->empty()) {
        ...
        w_assert1((*_direct_access_queue)[_front]._next
                                           == _invalid_key);
        w_assert1((*_direct_access_queue)[_front]._previous
                                           == _invalid_key);
        ...
        (*_direct_access_queue)[old_front_entry._next]._previous
                                             = _invalid_key;
...
bool hashtable_queue<key>::remove(key k) {
    if (!this->contains(k)) {
        ...
        if (old_key._next != _invalid_key) {
            (*_direct_access_queue)[old_key._next]._previous
                                             = old_key._previous;
        } else {
            _back = old_key._previous;
        }
        if (old_key._previous != _invalid_key) {
            (*_direct_access_queue)[old_key._previous]._next
                                             = old_key._next;
        } else {
